...
ggplot(sim, aes(x = t)) +
  geom_smooth(aes(y = y), linewidth = 1.5, color = "blue") +
...
ggplot(reshaped_data, aes(x = t)) +
  geom_smooth(aes(y = y_t1), linewidth = 1.5, color = "blue", se=FALSE) +
  geom_smooth(aes(y = y_t2), linewidth = 1.5, color = "blue", se=FALSE) +
...
$$y(t)=e^{\lambda t}$$
$$y(t)=e^{(-1 \pm \sqrt{3}i)t}$$
$$y(t)=e^{-t}e^{\sqrt{3}ti} \tag{1}$$

```{r difference5, echo = F, warning=F, message=F, results = "hide", fig.keep='last', fig.cap = "Complex Exponential Function $e^{iθ}$", fig.pos = "h"}
...
$$
$$Z(t)=e^{-t}(cos(\sqrt{3}t)-isin(\sqrt{3}t) \tag{3}$$
Equation (3) is the conjugate of equation (2).
